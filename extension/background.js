...
async function handleSaveComponent(payload, sender) {
    validatePayload(payload);
    const windowId = sender?.tab?.windowId;
    const screenshotDataUrl = await chrome.tabs.captureVisibleTab(windowId, { format: "png" });
    const croppedDataUrl = await cropScreenshotToBounds(screenshotDataUrl, payload.bounds, payload.devicePixelRatio);
    const record = {
...
function validatePayload(payload) {
    ...
    if (typeof payload.devicePixelRatio !== "number" ||
        Number.isNaN(payload.devicePixelRatio) ||
        payload.devicePixelRatio <= 0) {
        throw new Error("Invalid device pixel ratio.");
    }
}
async function cropScreenshotToBounds(sourceDataUrl, bounds, devicePixelRatio) {
